...
function TabIcon({ name, focused }: { name: string; focused: boolean }) {
  ...
  const color = focused ? '#B388FF' : isHovered ? '#FFFFFF' : '#555555';
...
function SlidingTabBar({ state, descriptors, navigation }: any) {
  ...
  // Handle max-width constraints if any, else use raw width
      ...
      {state.routes.map((route: any, index: number) => {
        ...
        return (
          ...
          >
            ...
            <Text style={[styles.tabLabel, { color: isFocused ? '#B388FF' : '#555555' }]}>
